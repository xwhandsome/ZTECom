...
QUERY_CONTEXT_ALIASES = {
    "发烧": ["退烧药", "对乙酰氨基酚", "acetaminophen", "paracetamol", "发热"],
    "发热": ["退烧药", "对乙酰氨基酚", "acetaminophen", "paracetamol"],
    "头痛": ["退烧药", "对乙酰氨基酚", "疼痛"],
    "疼痛": ["退烧药", "对乙酰氨基酚", "疼痛"],
    "血压": ["降压药", "氨氯地平", "amlodipine", "高血压"],
    "高血压": ["降压药", "氨氯地平", "amlodipine"],
}
...
class CareAgent:
    ...
    def chat(self, session_id: str, user_text: str, mode: str = "health_assistant") -> dict[str, Any]:
        ...
        if result.intent == "knowledge_query":
            reply, refs, rag_llm_used, rag_llm_status = self._answer_with_rag(user_text, state)
            if rag_llm_used:
                llm_used = True
            llm_status = rag_llm_status
    ...
    def _normalize_llm_slots(
        ...
        aliases = {
            ...
            "brightness": ["brightness", "light_brightness", "percent", "brightness_percent"],
            "brightness_delta": ["brightness_delta", "light_delta"],
        ...
        for canonical, names in aliases.items():
            ...
            if canonical == "medicine":
                ...
                brightness = self._normalize_percent(value)
                if brightness is not None:
                    slots["brightness"] = brightness
                    slots.setdefault("action", "set")
            elif canonical == "brightness_delta":
                delta = self._normalize_signed_number(value)
                if delta is not None:
                    slots["brightness_delta"] = delta
                    slots.setdefault("action", "set")
        ...
        if "time" not in slots:
            ...
            if time_info:
                slots.update(time_info)
        if slots.get("device") == "灯":
            brightness = extract_brightness(user_text)
            if brightness is not None:
                slots["brightness"] = brightness
                slots.setdefault("action", "set")
            brightness_delta = extract_brightness_delta(user_text)
            if brightness_delta is not None:
                slots["brightness_delta"] = brightness_delta
                slots.setdefault("action", "set")
    ...
    def _normalize_medicine_value(self, value: str) -> str | None:
        value = value.strip(" ，。,.！？!?\"“”")
        if any(word in value for word in ["什么药", "哪种药", "啥药", "吃什么", "用什么"]):
            return None
        ...
        if any(word in value for word in ["什么", "哪种", "啥"]):
            return None
    ...
    def _normalize_percent(self, value: Any) -> int | None:
        if isinstance(value, (int, float)):
            number = int(value)
        else:
    ...
    def _normalize_signed_number(self, value: Any) -> int | None:
        if isinstance(value, (int, float)):
            return int(value)
        match = re.search(r"-?\d{1,3}", str(value))
        return int(match.group(0)) if match else None
    ...
    def _answer_with_rag(self, user_text: str, state: SessionState) -> tuple[str, list[dict[str, Any]], bool, dict[str, Any]]:
        context_terms = []
        ...
        if not context_medicine:
            for trigger, aliases in QUERY_CONTEXT_ALIASES.items():
                if trigger in user_text:
                    context_terms.extend(aliases)
        refs = self.rag.search(user_text, context_terms=context_terms)
        llm_status = self.llm.status().to_dict()
        if not refs:
            return "本地知识库没有相关内容。我不能编造医疗建议，建议以医生或药品说明书为准。", [], False, llm_status

        ref_dicts = [ref.to_dict() for ref in refs]
        fallback = self._compose_rag_reply(user_text, refs, context_medicine)
        context_hint = None
        reply_prefix = ""
        if context_medicine and "这个药" in user_text:
            ...
            context_hint = f"用户说“这个药”时，按最近提醒理解为：{display_medicine}。"
            reply_prefix = f"这里的“这个药”按最近提醒理解为：{display_medicine}。"

        llm_refs = ref_dicts[:1] if context_medicine else ref_dicts
        answer, status = self.llm.synthesize_rag_answer(user_text, llm_refs, context_hint=context_hint)
        if answer:
            return reply_prefix + answer, ref_dicts, True, status.to_dict()
        return fallback, ref_dicts, False, status.to_dict()
    ...
    def _reply_for_events(self, result: IntentResult, events: list[dict[str, Any]], short: bool = False) -> str:
        ...
        if result.intent == "control_device":
            ...
            if device.get("device") == "灯":
                brightness = f"，亮度{device.get('brightness', 0)}%"
                return f"已更新{device['room']}{device['device']}：{device['status']}{brightness}。"
