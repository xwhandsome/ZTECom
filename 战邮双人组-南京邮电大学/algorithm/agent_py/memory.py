...
def build_default_state(session_id: str) -> SessionState:
    return SessionState(
        ...
        device_state={
            ...
            "卧室:灯": {"room": "卧室", "device": "灯", "status": "off", "brightness": 0},
            "客厅:空调": {"room": "客厅", "device": "空调", "status": "off", "target_temp": None},
            "客厅:灯": {"room": "客厅", "device": "灯", "status": "on", "brightness": 80},
        },
        sensors={
...
class MemoryStore:
    ...
    def _ensure_schema_columns(self, conn: sqlite3.Connection) -> None:
        device_columns = {item["name"] for item in conn.execute("PRAGMA table_info(device_states)").fetchall()}
        if "brightness" not in device_columns:
            conn.execute("ALTER TABLE device_states ADD COLUMN brightness INTEGER")
    ...
    def _load_state(self, conn: sqlite3.Connection, session: sqlite3.Row) -> SessionState:
        ...
        device_state = {
            ...
            for row in conn.execute(
                """
                SELECT room, device, status, target_temp, brightness
                FROM device_states
                WHERE session_id = ?
        ...
        for device in device_state.values():
            if device.get("device") == "灯" and device.get("brightness") is None:
                device["brightness"] = 80 if device.get("status") == "on" else 0
        sensors = {
            row["room"]: {
    ...
    def _save_devices(self, conn: sqlite3.Connection, state: SessionState, ts: str) -> None:
        for key, device in state.device_state.items():
            ...
            if name == "灯" and brightness is None:
                brightness = 80 if device.get("status") == "on" else 0
            conn.execute(
                """
                INSERT INTO device_states(id, session_id, room, device, status, target_temp, brightness, updated_at)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                """,
                (
