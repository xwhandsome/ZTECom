...
QUESTION_MEDICINE_WORDS = ["什么药", "哪种药", "啥药", "什么退烧药", "什么降压药", "用什么", "吃什么"]
...
def extract_medicine(text: str) -> str | None:
    ...
    if not medicine or medicine in {"药", "这个药"}:
        return None
    if any(word in medicine for word in QUESTION_MEDICINE_WORDS) or medicine in {"什么", "哪种", "啥"}:
        return None
    if len(medicine) > 16:
...
def extract_action(text: str) -> str | None:
    ...
    if any(word in text for word in ["打开", "开启", "开空调", "开灯", "自动开"]):
        return "on"
    if any(word in text for word in ["调到", "设到", "设为", "设置到", "亮度", "调亮", "调暗", "亮一点", "暗一点"]):
        return "set"
    return None
...
def extract_brightness(text: str) -> int | None:
    patterns = [
        r"(?:亮度|调到|设到|设置到|设为|到|为)\s*(?P<value>\d{1,3})\s*%",
        r"(?P<value>\d{1,3})\s*%\s*(?:亮度|的亮度)?",
        r"亮度\s*(?P<value>\d{1,3})",
    ]
...
def extract_brightness_delta(text: str) -> int | None:
    if any(word in text for word in ["调亮", "亮一点", "更亮"]):
        return 20
    if any(word in text for word in ["调暗", "暗一点", "更暗"]):
        return -20
    return None
...
class RuleNLU:
    def analyze(self, text: str, state: SessionState) -> IntentResult:
        ...
        if extract_device(text) and any(word in text for word in ["打开", "关闭", "关掉", "开启", "调到", "设为", "设置", "亮度", "调亮", "调暗", "亮一点", "暗一点"]):
            slots = self.extract_generic_slots(text, state)
    ...
    def extract_generic_slots(self, text: str, state: SessionState | None = None) -> dict[str, Any]:
        ...
        if brightness is not None:
            ...
            if "action" not in slots:
                slots["action"] = "set"
        brightness_delta = extract_brightness_delta(text)
    ...
    def _looks_like_knowledge(self, text: str) -> bool:
        ...
        if any(word in text for word in ["提醒我", "提醒奶奶", "打开", "关闭"]):
            return False
        if "通知" in text and not any(word in text for word in ["为什么", "规则", "说明", "注意", "确认"]):
            return False
        return True
