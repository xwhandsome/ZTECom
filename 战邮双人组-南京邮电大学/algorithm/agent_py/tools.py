    ...
    def _handle_control_device(self, state: SessionState, args: dict[str, Any]) -> dict[str, Any]:
        ...
        if device.get("device") == "灯":
            if args.get("brightness") is not None:
                device["brightness"] = max(0, min(100, int(args["brightness"])))
            elif args.get("brightness_delta") is not None:
                current = int(device.get("brightness") or (80 if device.get("status") == "on" else 0))
                device["brightness"] = max(0, min(100, current + int(args["brightness_delta"])))
            elif action == "on" and device.get("brightness") is None:
                device["brightness"] = 80
            elif action == "off":
                device["brightness"] = 0
            if device.get("brightness", 0) > 0 and action != "off":
                device["status"] = "on"
