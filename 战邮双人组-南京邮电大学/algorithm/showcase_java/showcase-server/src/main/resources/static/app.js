...
function bindActions() {
  ...
  byId("toolEventRows").addEventListener("click", handleTaskAction);
...
function renderDevices(devices) {
  ...
  values.forEach((device) => {
    ...
    if (device.device === "灯") {
      detail.textContent = device.brightness == null ? "亮度 -" : `亮度 ${device.brightness}%`;
    } else {
...
function renderAssistant() {
  ...
  setText("analysisLlm", formatLlmUsage(response));
...
function formatLlmUsage(response) {
  if (!response || !response.llm_used) {
    return "未调用";
  }
  if (response.intent === "knowledge_query") {
    return "RAG归纳";
  }
  return "槽位兜底";
}
...
function renderTasks() {
  ...
  const eventBody = byId("toolEventRows");
  clear(body);
  clear(eventBody);
  const session = state.session || {};
  const rows = [];
  ...
  if (!rows.length) {
    body.appendChild(emptyRow("暂无提醒、规则或待确认事项", 5));
  } else {
    rows.forEach((item) => body.appendChild(taskRow(item)));
  }

  const toolEvents = (session.recent_tool_events || []).slice(-8).reverse();
  setText("toolEventCount", `${toolEvents.length} 条`);
  if (!toolEvents.length) {
    eventBody.appendChild(emptyRow("暂无工具事件", 4));
  } else {
    toolEvents.forEach((event) => eventBody.appendChild(toolEventRow(event)));
  }
}
...
function isProxyProblem(result) {
  return result && (result.status === "python_offline" || result.status === "python_bad_response" || result.status === "python_timeout");
}
...
function toolEventRow(event) {
  const tr = document.createElement("tr");
  [event.tool_name, event.success ? "成功" : "失败", event.ts || "-"].forEach((value) => {
    const td = document.createElement("td");
    td.textContent = value == null ? "-" : String(value);
    tr.appendChild(td);
  });
  ...
  button.dataset.taskAction = "view-event";
  button.dataset.payload = JSON.stringify(event);
